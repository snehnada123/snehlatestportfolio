      ...
      <div className="mx-auto max-w-6xl page-inner flex flex-col gap-6 sm:gap-10">
        ...
        <div className="flex flex-col gap-4">
          {education.map((entry) => (
            <article
              key={entry.id}
              className="flex flex-col gap-2 border border-border p-4 sm:flex-row sm:items-start sm:justify-between sm:p-6"
            >
              <div className="flex flex-col gap-1">
                <h3 className="text-headline">{entry.school}</h3>
                <p className="text-body text-muted">{entry.detail}</p>
              </div>
              <span className="text-mono text-caption text-muted whitespace-nowrap">
                {entry.period}
              </span>
            </article>
          ))}
